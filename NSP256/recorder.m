...
%tic
%total_size = size(event_data)
%for i = 1:total_size(1)
%    data = event_data{i,2};
%    shape = int16(length(data))
%   buffer = int16(zeros(1, 2*shape + 1));
%    ret = typecast(shape, 'int16');
%    ecr = typecast(data, 'int16');
%    buffer(1,1) = ret;
%    buffer(1,2:end) = ecr;
%    zeta = 0
%end 
...
%EncoderPlz('send', event_data, cont);
...
while(trial_length > toc(current_time))
    ...
    % after, for each y channel, the event data is encoded as follows (i
    % also think the only "event" is a spike, so we're assuming the only
    % events are spikes)

    %in channel order: int16 for the length of the following vector is x 
    % x * uint32s = 2x int16 bytes of data, 
    % then the next channel, the x can equal 0. 
